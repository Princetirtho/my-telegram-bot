...
async def init_db(application: Application):
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        # কোর্স কস্ট ডায়নামিক করার জন্য কনফিগ টেবিল তৈরি
        await db.execute('''
            CREATE TABLE IF NOT EXISTS config (
                key TEXT PRIMARY KEY,
                value TEXT
            )''')
        # ডিফল্ট কোর্স কস্ট ২০ পয়েন্ট সেট করা (যদি আগে থেকে না থাকে)
        await db.execute('''
...
# --- ডায়নামিক কোর্স কস্ট রিড করার ফাংশন ---
async def get_course_cost() -> float:
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT value FROM config WHERE key = 'course_cost'") as cursor:
            ...
            if row:
                return float(row[0])
    return 20.0  # কোনো কারণে এরর হলে ডিফল্ট ২০ পয়েন্ট
...
async def handle_text_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "👤 প্রোফাইল":
        ...
        # মেম্বারশিপ নেওয়ার জন্য পয়েন্ট চেক করা
        ...
        if user_id == ADMIN_ID:
            await update.message.reply_text(f"🔍 [DEBUG] কোর্স কি: `{course_key}`")
        
        cost = await get_course_cost()
        async with aiosqlite.connect(DB_NAME) as db:
            # চেক করা ইউজার ইতিমধ্যে এই কোর্সে জয়েন করেছে কিনা
            ...
            # শেষ মুহূর্তে আরও একবার পর্যাপ্ত পয়েন্ট ব্যালেন্স চেক করা
            async with db.execute("SELECT points FROM users WHERE user_id = ?", (user_id,)) as cursor:
            ...
            if current_points < cost:
                await update.message.reply_text(
                    f"❌ **দুঃখিত! কোর্সটি কিনতে আপনার পর্যাপ্ত পয়েন্ট নেই।**\n\n"
                    f"• কোর্স কিনতে প্রয়োজন: `{cost:.1f}` পয়েন্ট\n"
                    f"• আপনার বর্তমান পয়েন্ট: `{current_points:.1f}` পয়েন্ট"
                )
                return
        ...
        # ========== ইউজার-স্পেসিফিক ওয়ান-টাইম লিংক ==========
        final_link = None
        ...
        try:
            ...
            # ইউজার ডেটা সেভ এবং পয়েন্ট কেটে নেওয়া (কোর্স পারচেজ প্রসেস)
            async with aiosqlite.connect(DB_NAME) as db:
                # পয়েন্ট ডিডাক্ট করা
                await db.execute(
                ...
                # ইউজার অ্যাক্সেস ডাটা সেভ
                await db.execute(
                    "INSERT INTO user_access (user_id, course_key, invite_link, used) VALUES (?, ?, ?, ?)",
                    (user_id, course_key, final_link, 0)
                )
                await db.commit()
            
            if user_id == ADMIN_ID:
                await update.message.reply_text(
                    f"✅ [ADMIN] লিংক তৈরি সফল!\n"
                    f"👤 ইউজার: {user_id}\n"
                    f"🔗 {final_link}"
                )
...
async def auto_approve_join_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_join_request.invite_link and hasattr(chat_join_request.invite_link, 'invite_link'):
        ...
        logger.warning(f"No invite link found in join request for user {user_id}")
        try:
    ...
    logger.info(f"🔍 লিংক রিকোয়েস্ট পেয়েছি: {invite_link}")
    logger.info(f"👤 ইউজার আইডি: {user_id}")
    
    try:
        async with aiosqlite.connect(DB_NAME) as db:
            # ডেটাবেজে লিংক খোঁজা
            async with db.execute(
                "SELECT user_id, course_key FROM user_access WHERE invite_link = ? AND used = 0",
                (invite_link,)
            ) as cursor:
                row = await cursor.fetchone()
            
            if not row:
                logger.info(f"❌ লিংক ডেটাবেজে নেই অথবা ইতিমধ্যে ব্যবহৃত: {invite_link}")
                try:
                    await chat_join_request.decline()
                except:
                    pass
            ...
            if original_user_id != user_id:
                logger.info(f"❌ ভুল ইউজার ট্রাই করছে! আসল: {original_user_id}, ক্লিক করেছে: {user_id}")
                try:
                    await chat_join_request.decline()
                except:
                    pass
            ...
            # রিকোয়েস্ট অ্যাপ্রুভ
            await chat_join_request.approve()
            
            # লিংক এক্সপায়ার করা
            await db.execute(
                "UPDATE user_access SET used = 1 WHERE user_id = ? AND invite_link = ?",
                (user_id, invite_link)
            )
            await db.commit()
...
async def admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "💡 **কোর্স কি ফরম্যাট:**\n"
        "• hsc27_acs_phy_cycle1\n"
        "• hsc27_bp_chem_combo\n"
        "• hsc28_udvas_ebi_link"
    )
    await update.message.reply_text(msg, parse_mode="Markdown")

# --- ডাইনামিক কোর্স পয়েন্ট সেটআপ কমান্ড ---
async def set_needpoint(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            await db.execute(
                "INSERT OR REPLACE INTO config (key, value) VALUES ('course_cost', ?)",
                (str(new_cost),)
            )
            await db.commit()
...
# --- এডমিন কর্তৃক কার কাছে কোন কোর্স আছে তা দেখার কমান্ড ---
async def view_user_courses(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            ...
            # ইউজারের এক্সেস করা কোর্সসমূহ জানা
            async with db.execute(
                "SELECT c.course_name, c.course_key, ua.used "
                "FROM user_access ua "
                "JOIN courses c ON ua.course_key = c.course_key "
                "WHERE ua.user_id = ?",
                (target_user_id,)
            ) as cursor:
                rows = await cursor.fetchall()
...
# --- এডমিন কর্তৃক রেফারাল লিস্ট দেখার কমান্ড ---
async def view_referrals(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        # সফল রেফারাল ট্র্যাক
        query = (
            "SELECT r.referrer_id, u1.full_name, COUNT(r.referee_id) as total "
            "FROM referrals r "
...
async def save_add_img(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "INSERT INTO courses (course_key, course_name, image_id, info_text, channel_id) VALUES (?, ?, ?, ?, ?)",
            (data['key'], data['name'], img_id, data['info'], data['channel'])
        )
        await db.commit()
...
async def skip_add_img(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "INSERT INTO courses (course_key, course_name, image_id, info_text, channel_id) VALUES (?, ?, ?, ?, ?)",
            (data['key'], data['name'], None, data['info'], data['channel'])
        )
        await db.commit()
